...

{
	...
	public void drawBoard(Graphics g) {
	    ...
	    for (int i = 0; i <= LARGEUR; i++) {
	    	g.drawString("" + (char) ('A' + i), i * TAILLEPIONS+26, 50+TAILLEPIONS*(HAUTEUR+1));
	    }
	    for (int i = 0; i <= HAUTEUR; i++) {
	    	g.drawString("" + (i+1), 50+TAILLEPIONS*(HAUTEUR+1), (i) * TAILLEPIONS + 35);
	    }
		...
		for (int j = 0; j < TAILLEPIONS * HAUTEUR; j += TAILLEPIONS) {
			...
			g.fillRect(bx + i - TAILLEPIONS, by + j - TAILLEPIONS, TAILLEPIONS, TAILLEPIONS);
			g.fillRect(bx + i + TAILLEPIONS, by + j + TAILLEPIONS, TAILLEPIONS, TAILLEPIONS);
			g.fillRect(bx + i - TAILLEPIONS, by + j + TAILLEPIONS, TAILLEPIONS, TAILLEPIONS);
			g.fillRect(bx + i + TAILLEPIONS, by + j - TAILLEPIONS, TAILLEPIONS, TAILLEPIONS);
		}
		...
		for (int j = 0; j < TAILLEPIONS * HAUTEUR; j += TAILLEPIONS) {
		    ...
		    //test the drawing of a piece
		    //g.fillOval(bx + TAILLEPIONS * i - 10 , by + TAILLEPIONS * j - 10, TAILLEPIONS - 12, TAILLEPIONS - 12);
	    ...
	    for (int i = 0; i < TAILLEPIONS * LARGEUR; i += TAILLEPIONS) {
		for (int j = 0; j < TAILLEPIONS * HAUTEUR; j += TAILLEPIONS) {
		    g.drawRect(bx + i, by + j, TAILLEPIONS, TAILLEPIONS);	
		}
		}
		...
		for (int j = 0; j <= HAUTEUR; j++) {
			...
			g.fillOval(bx + TAILLEPIONS * i - 15, by + TAILLEPIONS * j - 15, TAILLEPIONS - 15,
				TAILLEPIONS - 15);
			break;		    
		    case (NOIR):
			g.setColor(new Color(0, 0, 0));
